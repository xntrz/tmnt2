...
, m_Color({ 0xEE, 0xEE, 0xEE, 0x28 })
{
    ...
    m_fLength = CScreen::VirtualPixelW() * 150.0f;
...
, m_Color({ 0xFF, 0xFF, 0xFF, 0x23 })
{
    ...
    float w = 1.0f;// CScreen::VirtualPixelW();
    float h = 1.0f;//CScreen::VirtualPixelH();

    m_vVelocityMax.x = w * 0.0f;
    m_vVelocityMax.y = h * 1800.0f;
    m_fWidth = w * 100.0f;
    m_fHeight = h * 300.0f;
    m_fRandomW = w * 25.0f;
    m_fRandomH = h * 75.0f;
...
void CScrEffectRainBack::Generate(PARTICLE* pParticle)
{
    ...
    if (fScale > 0.3f)
        pParticle->m_vSize.x = m_fWidth;
    else
        pParticle->m_vSize.x = m_fWidth * 0.5f;

...
, m_bPause(true)
{
    ...
    RwInt32 hierarchyFlags = RpHAnimHierarchyGetFlags(pHierarchy);
    ...
    RpHAnimHierarchySetKeyFrameCallBacks(m_pOutHierarchy, 1);
...
void CMotionController::SetMotion(CMotion* pMotion, float fStartTime, float fEndTime)
{
    ...
    if (fStartTime > 0.0f)
        RpHAnimHierarchyAddAnimTime(pHierarchy, fStartTime);
...
void CMotionController::UpdateMotion(float dt)
{
    ...
    if (dt > 0.0f)
        RpHAnimHierarchyAddAnimTime(pHierarchy, dt);
};
...
void CMotionController::SetBlendMotion(CMotion* pMotion, float fStartTime, float fEndTime, float fBlendTime)
{
    ...
    RpHAnimHierarchySetCurrentAnim(m_pInHierarchy, pAnimation);
...
void CMotionController::UpdateBlendMotion(float dt)
{
    ...
    if (m_fTime < m_fBlendTime)
    {
        ...
        RpHAnimHierarchyBlend(m_pOutHierarchy, pHierarchy, m_pInHierarchy, dt);
    }
    else
    {

...
public:
    ...
    ~CMapCollisionModelContainer(void);
    void Period(void);
    uint32 RegistCollisionModel(RpClump* pClump, const char* pszName = nullptr, MAPTYPES::GIMMICKTYPE type = MAPTYPES::GIMMICKTYPE_NORMAL);
    void RemoveCollisionModel(uint32 hAtari);
    void UpdateCollisionModel(uint32 hAtari, RwV3d* pVelocity, RwV3d* pRotation);
...
/*static*/ RpAtomic* CMapCollisionModelContainer::CheckModelCollisionCallback(RpAtomic* atomic, void* pData)
{
    ...
    COLLISIONWORK* pWork = (COLLISIONWORK*)pData;
...
{
    ASSERT(pszName);
    ASSERT(nMaterialID >= 0 && nMaterialID < RpGeometryGetNumMaterialsMacro(pGeometry));
...
/*static*/ MAPTYPES::ATTRIBUTE CMapCollisionModelContainer::GetMapAttribute(RpCollisionTriangle* pCollTriangle, RpGeometry* pGeometry, RwRGBA& rColor)
{
    ASSERT(pCollTriangle->index >= 0 && pCollTriangle->index < pGeometry->numTriangles);
...
/*static*/ RpCollisionTriangle* CMapCollisionModelContainer::CollisionPointCallback(RpIntersection* pIntersection, RpCollisionTriangle* pCollTriangle, float fDistance, void* pData)
{
    CMapCollisionModel::COLLISIONPARAM_MODEL* pCollisionParam = (CMapCollisionModel::COLLISIONPARAM_MODEL*)pData;
...
/*static*/ RpCollisionTriangle* CMapCollisionModelContainer::CollisionLineCallback(RpIntersection* pIntersection, RpCollisionTriangle* pCollTriangle, float fDistance, void* pData)
{
    CMapCollisionModel::COLLISIONPARAM_MODEL* pCollisionParam = (CMapCollisionModel::COLLISIONPARAM_MODEL*)pData;
...
/*static*/ RpCollisionTriangle* CMapCollisionModelContainer::CollisionSphereCallback(RpIntersection* pIntersection, RpCollisionTriangle* pCollTriangle, float fDistance, void* pData)
{
    CMapCollisionModel::COLLISIONPARAM_MODEL* pCollisionParam = (CMapCollisionModel::COLLISIONPARAM_MODEL*)pData;
    ...
    if (Intersection::PointWithinTriangle(&vProjPos, pCollTriangle->vertices, &pCollTriangle->normal))
    {
        ...
        if (fDist < pCollisionParam->m_fDistance)
        {
            ...
            pCollisionParam->m_vNormal = pCollTriangle->normal;
            pCollisionParam->m_attribute = GetMapAttribute(pCollTriangle, pCollisionParam->m_pGeometry, pCollisionParam->m_Color);
        };
    }
    else
    {
...
/*static*/ RpCollisionTriangle* CMapCollisionModelContainer::CollisionBBoxCallback(RpIntersection* pIntersection, RpCollisionTriangle* pCollTriangle, float fDistance, void* pData)
{
    CMapCollisionModel::COLLISIONPARAM_MODEL* pCollisionParam = (CMapCollisionModel::COLLISIONPARAM_MODEL*)pData;
...
/*static*/ bool CMapCollisionModelContainer::CheckModelBoundingSphere(RwV3d* pvPos, float fRadius, RpIntersection* pIntersection)
{
    ASSERT(pvPos);
    ...
    if (pIntersection->type == rpINTERSECTLINE)
    {
    ...
    else if (pIntersection->type == rpINTERSECTSPHERE)
    {
...
uint32 CMapCollisionModelContainer::RegistCollisionModel(RpClump* pClump, const char* pszName, MAPTYPES::GIMMICKTYPE type)
{
    ASSERT(pClump);
    ASSERT(pszName);
...
void CMapCollisionModelContainer::UpdateCollisionModel(uint32 hAtari, RwV3d* pVelocity, RwV3d* pRotation)
{
    MODELINFO* pModelInfo = (MODELINFO*)hAtari;
    ASSERT(pModelInfo);
...
bool CMapCollisionModelContainer::CheckModelCollision(RpIntersection* pIntersection, RwV3d* pVelocity)
{
    ...
    for (int32 i = 0; i < COUNT_OF(m_aModelInfo); ++i)
    {
        ...
        if (CheckModelBoundingSphere(&RwFrameGetMatrixMacro(pFrame)->pos, pModelInfo->m_fRadius, pIntersection))
        {
...
/*static*/ void CMapCollisionModel::UpdateCollisionModel(uint32 hAtari, RwV3d* pVelocity, RwV3d* pRotation)
{

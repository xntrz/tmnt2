...
//
//  Sd loader common data
//
static char         SdHost[256];
static bool         SdLoaderCleanupPhase = false;
static int32        SdLoaderReadMode = SD_READMODE_HDD;


//
//  Sd loader callback commands
//
static int32 SdLoaderCallbackCommand[8];
static int32 SdLoaderCallbackParam[8];
static int32 SdLoaderLoadInfo = 0;
...
{
    int32 Bank = (int32)SdLoadTaskGetUserParam(_task);
    if (SdTransCodeTable[Bank].Count)
    {
        SdKdtTableData_t* TableData = (SdKdtTableData_t*)SdLoadTaskGetReadAddr(_task);
...
{    
    size_t systemFilenameLen = 16;
    size_t len = std::strlen(Host);

    if ((len + systemFilenameLen + 2) > sizeof(SdHost))
        return false;
    ...
    CFilename::ConvPathPlatform(SdHost);

    for (int32 i = 0; i < COUNT_OF(SdLoaderCallbackCommand); ++i)
    {
        SdLoaderCallbackCommand[i] = -1;
        SdLoaderCallbackParam[i] = -1;
    };
...
void SdLoaderTask(void)
{
    for (int32 i = 0; i < COUNT_OF(SdLoaderCallbackCommand); ++i)
    {
        if (SdLoaderCallbackCommand[i] != -1)
        {
            SdDrvLoaderCommandCB(SdLoaderCallbackCommand[i],
                                 SdLoaderCallbackParam[i]);
    ...
    SdLoaderLoadInfo &= (~SD_LOADINFO_COMMAND);
};


void SdLoad(int32 _command, int32 _param)
{
...
//        if (SdLoaderCallbackCommand[i] == -1)
//        {
//            SdLoaderCallbackCommand[i] = _command;
//            SdLoaderCallbackParam[i]   = _param;
//
//            SdLoaderLoadInfo |= SD_LOADINFO_COMMAND;
//
//            return;
...
void SdSetReadMode(int32 mode)
{
    SdLoaderReadMode = mode;
};


int32 SdGetReadMode(void)
{
    return SdLoaderReadMode;
};


int32 SdGetLoadInfo(void)
{
    if (SdWavLoadTaskCnt > 0)
        return SD_LOADINFO_WAVE;

    if (SdTransWavLoadTaskCnt > 0)
        return SD_LOADINFO_TRANS_WAVE;

    return SdLoaderLoadInfo;
};
...
void SdMakeFilename(char* _buff, const char* _filename)
{
    ...
    CFilename::ConvPathPlatform(_buff);
};
...
void SdStreamLoad(void)
{
    ...
    for (int32 i = 0; i < SdVoxLoadNum; ++i)
    {
        ...
        if (SdLoadTaskReadSync(Filename, VoxLoadAddr, SD_SCT_SIZE, 0))
        {
            ...
            if (StrHdr->Size > SD_SCT_SIZE)
                SdLoadTaskReadSync(Filename, VoxLoadAddr, StrHdr->Size, 0);
    ...
    for (int32 i = 0; i < SdVagLoadNum; ++i)
    {
        ...
        if (SdLoadTaskReadSync(Filename, VagLoadAddr, SD_SCT_SIZE, 0))
        {
            ...
            if (StrHdr->Size > SD_SCT_SIZE)
                SdLoadTaskReadSync(Filename, VagLoadAddr, StrHdr->Size, 0);
...
void SdSystemDataLoadInfo(void)
{
    ...
    if (Buff)
    {
        ...
        if (SdLoadTaskReadSync(Filename, Buff, SD_SCT_SIZE, 0))
    ...
    if (Buff)
    {
        ...
        if (SdLoadTaskReadSync(Filename, Buff, SD_SCT_SIZE, 0))
        {
            ...
            if (((void*)(uint32(SeDataAddr) + SeDataSize)) < SdHeapAddrEnd)
            {
                ...
                if (SdLoadTaskReadSync(Filename, SeDataAddr, SeDataSize, SeDataOffset))
    ...
    if (Buff)
    {
        ...
        if (SdLoadTaskReadSync(Filename, Buff, SD_SCT_SIZE * 3, 0))
        {
            ...
            if (((void*)(uint32(WavDataAddr) + WavDataSize)) < SdHeapAddrEnd)
            {
                ...
                if (SdLoadTaskReadSync(Filename, WavDataAddr, WavDataSize, WavDataOffset))
...
void SdTransLoad(int32 _bank)
{
    ...
    //
    void* hTask = SdLoadTaskReadAsync(Filename, Address, Size, SdTransCodeTable[_bank].Offset);
    if (hTask)
    {
        SdLoadTaskSetUserParam(hTask, (void*)_bank);
...
void SdWaveLoad(int32 _bank, uint32 _address)
{
    ...
    //  Regist async load task
    //
    void* hTask = SdLoadTaskReadAsync(Filename, (void*)_address, Size, SdEnvelopeTable[_bank].WaveOffset);
    if (hTask)
    {

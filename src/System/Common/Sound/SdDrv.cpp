...
#if defined(TMNT2_SOUND_DSOUND)
#include "Backend/DSound/SdDS.hpp"
#elif defined(TMNT2_SOUND_OPENAL)
#include "Backend/OpenAL/SdAL.hpp"
#else
#error Unknown sound backend
#endif    


#define SDDRV_SETCODE_QUEUE_SIZE (48)
#define SDDRV_USECODE_QUEUE_SIZE (32)
...
static double SdDrvTimerAccumulator = 0.0;
static double SdDrvTimerPrevT = 0.0;
static SdMutex_t* SdDrvSetSoundCodeQueueMutex;
...
static void SdDrvActivateCtx(void)
{
#if defined(TMNT2_SOUND_OPENAL)
    SdALActivateCtx();
#endif /* defined(TMNT2_SOUND_OPENAL) */
};


static void SdDrvDeactivateCtx(void)
{
#if defined(TMNT2_SOUND_OPENAL)
    SdALDeactivateCtx();
#endif /* defined(TMNT2_SOUND_OPENAL) */
...
{
    SdMutexLock(SdDrvSetSoundCodeQueueMutex);
    SdQueueMove(SdDrvSetSoundCodeQueueW, SdDrvSetSoundCodeQueueR);
    SdMutexUnlock(SdDrvSetSoundCodeQueueMutex);
...
static void SdDrvUpdateAll(void)
{
    SdLoaderTask();
    ...
    SdDrvTimerAccumulator += ElapsedT;

    if (SdDrvTimerAccumulator > PeriodMax)
        SdDrvTimerAccumulator = PeriodMax;

    while (SdDrvTimerAccumulator >= PeriodT)
    {
        SdSeqMain();
        SdIOSetTask();
        SdSeqPlyTask();
        ...
        SdDrvTimerAccumulator -= PeriodT;
    };

    SdDrvTimerPrevT = NowT;
...
static void SdDrvSoundThreadCallback(void* _param)
{
    SdDrvTask();
};


bool SdDrvInitialize(void* _window, const char* _path, void* _heap, size_t _heapSize, int32 _option)
{
    //
    //	Init sd drv option
    //
    ...
    bool SdLogEnabled = (SdDrvOptFlag & SD_OPTFLAG_LOG) ? true : false;
    bool SdLogLoad = (SdDrvOptFlag & SD_OPTFLAG_LOG_LOAD) ? true : false;
    SdLogInitialize(SdLogEnabled, SdLogLoad);
    ...
    bool bInit = false;
#if defined(TMNT2_SOUND_DSOUND)
    bInit = SdDSInitialize((HWND)_window);
#elif defined(TMNT2_SOUND_OPENAL)
    bInit = SdALInitialize();
#else
#error Unknown sound backend
#endif    
    if (!bInit)
    ...
    /* init queue mutex */
    SdDrvSetSoundCodeQueueMutex = SdMutexCreate();
    if (!SdDrvSetSoundCodeQueueMutex)
        return false;

    /* init set sound code queues */
    SdDrvSetSoundCodeQueueR = SdQueueCreate(SDDRV_SETCODE_QUEUE_SIZE, sizeof(SdSetSoundCode_t));
    if (!SdDrvSetSoundCodeQueueR)
        return false;
    
    SdDrvSetSoundCodeQueueW = SdQueueCreate(SDDRV_SETCODE_QUEUE_SIZE, sizeof(SdSetSoundCode_t));
    if (!SdDrvSetSoundCodeQueueW)
        return false;

    /* init set use sound code queue */
    SdDrvSetUseSoundCodeQueue = SdQueueCreate(SDDRV_USECODE_QUEUE_SIZE, sizeof(SdSetSoundCode_t));
    ...
    //  Init sd drv thread
    //
    SdDrvTimerPrevT = SdTimerGetNow();

    if (!SdThreadInitialize(&SdDrvSoundThreadCallback, nullptr))
        return false;
...
#define SDDRV_QUEUE_DESTROY(q)  \
    ...
    if (SdDrvSetSoundCodeQueueMutex)
    {
        SdMutexDestroy(SdDrvSetSoundCodeQueueMutex);
        SdDrvSetSoundCodeQueueMutex = nullptr;
    };
...
bool LoadSEBank(int32 _bank)
{
    ...
    /* skip loading first two banks due some of SE depends on each other
       (for example some SE from bank 1 depends on wavedata from bank 0) */

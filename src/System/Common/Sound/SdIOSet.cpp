...
static void SdIODispatchADSR(void)
{
    for (int32 i = 0; i < COUNT_OF(SdIOPack.Adsr); ++i)
    {
        ...
        {
            Adsr->Sl &= 0x7F;
            SdSeqPlySetSL(i, Adsr->Sl);
        };

        if (Adsr->Ar & 0x80)
        {
            Adsr->Ar &= 0x7F;
            SdSeqPlySetAR(i, Adsr->Ar);
        };

        if (Adsr->Dr & 0x80)
        {
            Adsr->Dr &= 0x7F;
            SdSeqPlySetDR(i, Adsr->Dr);
        };

        if (Adsr->Sr & 0x80)
        {
            Adsr->Sr &= 0x7F;
            SdSeqPlySetSR(i, Adsr->Sr);
        };

        if (Adsr->Rr & 0x80)
        {
            Adsr->Rr &= 0x7F;
            SdSeqPlySetRR(i, Adsr->Rr);
        };
    };	
};
...
static void SdIODispatchMessage(void)
{
    ...
    {    
        ...
        case SDIO_MESSAGE_TYPE_FREQ:
            SdSeqPlySetFreq(SdIOPack.Message[i].Channel, SdIOPack.Message[i].Value);
            break;

        case SDIO_MESSAGE_TYPE_ADDR:
            SdSeqPlySetAddress(SdIOPack.Message[i].Channel,
                              SdIOPack.Message[i].Code,
                              (SdWaveDataHdr_t*)SdIOPack.Message[i].Value);
            break;

        case SDIO_MESSAGE_TYPE_LVOL:
            SdSeqPlySetVolL(SdIOPack.Message[i].Channel, SdIOPack.Message[i].Value);
            break;

        case SDIO_MESSAGE_TYPE_RVOL:
            SdSeqPlySetVolR(SdIOPack.Message[i].Channel, SdIOPack.Message[i].Value);
            break;
